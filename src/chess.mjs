...
// Piece set: cburnett (Colin M.L. Burnett), CC BY-SA 3.0, used e.g. by lichess.org.
const PIECE_PATHS = {
...
function parsePlacement(placement) {
  ...
  // board[rank][file], rank 0 = rank 8 (top of a FEN string), file 0 = file a
  return rows.map((row) => {
    const squares = [];
    for (const ch of row) {
      if (/[1-8]/.test(ch)) {
        ...
        squares.push((ch === ch.toUpperCase() ? "w" : "b") + ch.toUpperCase());
      } else {
        throw new Error(`Invalid FEN: unexpected character "${ch}"`);
      }
    }
    if (squares.length !== 8) {
      throw new Error(`Invalid FEN: rank "${row}" does not have 8 files`);
    }
    return squares;
  });
}
...
export function renderChessBoard(fen, options = {}) {
  ...
  for (const row of board) for (const p of row) if (p) usedPieces.add(p);

  const defs = [...usedPieces]
  ...
  for (let r = 0; r < 8; r++) {
    for (let f = 0; f < 8; f++) {
      ...
      if (showRank || showFile) {
        const labelFill = isLight ? "var(--chess-dark, #b58863)" : "var(--chess-light, #f0d9b5)";
        if (showRank) {
          squaresSvg += `<text x="${x + 2.5}" y="${y + 8}" font-size="6" font-family="sans-serif" fill="${labelFill}">${rank + 1}</text>`;
        }
        if (showFile) {
          squaresSvg += `<text x="${x + SQ - 5}" y="${y + SQ - 2.5}" font-size="6" font-family="sans-serif" fill="${labelFill}" text-anchor="end">${FILES[file]}</text>`;
        }
      }
  ...
  const lichessLabel = options.lichessLabel || "Open in Lichess";
  const lichessLink =
    `<a class="chess-board__lichess" href="${lichessUrl}" target="_blank" rel="noopener noreferrer">${lichessLabel}</a>`;
